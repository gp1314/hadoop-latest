...
public class TestGetJournalEditServlet {
  ...
  /**
   * A Kerberos-authenticated NameNode should be authorized using the complete
   * principal exposed by SPNEGO.
   *
   * @throws IOException for unexpected validation failures
   */
  @Test
  public void testSecurityRequestNameNodeUsesPrincipal() throws IOException {
    CONF.set(HADOOP_SECURITY_AUTHENTICATION, "kerberos");
    UserGroupInformation.setConfiguration(CONF);
    HttpServletRequest request = mock(HttpServletRequest.class);
    Principal principal = mock(Principal.class);
    when(request.getRemoteUser()).thenReturn("nn");
  ...
  @Test
  public void testSecurityRequestNameNodeUsesRemoteUserWithoutPrincipal()
      throws IOException {
    CONF.set(HADOOP_SECURITY_AUTHENTICATION, "kerberos");
    UserGroupInformation.setConfiguration(CONF);
    HttpServletRequest request = mock(HttpServletRequest.class);
    when(request.getRemoteUser()).thenReturn("nn/localhost@REALM.TLD");
